...
class Diffusion(nn.Module):
    ...
    def p_sample_loop(self, model, shape, skip_step):
        ...
        print("Diffusion.t_timesteps: ", Diffusion.t_timesteps, " ")
...
def main():
    ...
    # 데이터로더 생성
    flower_dataset = get_flower_dataset(image_size)
    dataloader = DataLoader(flower_dataset, batch_size=dataloader_batch_size, shuffle=True, drop_last=True)
    ...
    for epoch in tqdm(range(epochs)):
        ...
        for step, image_batch in enumerate(tqdm(dataloader)):
            ...
            ## loss = get_loss(model, image_batch[0], t, device)
            # print("image_batch len: ", len(image_batch))
            # print("image_batch[0].shape: ", image_batch[0].shape)

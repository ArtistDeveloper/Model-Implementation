...
#     arr = [
#         divisor
#     ] * groups  # divisor값을 갖는 요소를 groups의 횟수만큼 반복해서 list에 넣음. divisor: 3, groups: 4라면 [3, 3, 3, 3]이다.
#     if remainder > 0:
...
#         in_out = list(
#             zip(dims[:-1], dims[1:])
#         )  # syntax testing에서 한 번 값 넣고 테스트 해보기, in_out이 뭐할 때 쓰는 변수지?
...
#     out = alphas.gather(
#         -1, target_t.cpu()
#     )  # dim=-1은 차원에서 마지막 차원을 뜻한다.(무조건 열일듯), index: take로 취할 단일 텐서
...
#     def __init__(self, total_timesteps=300):
#         """
...
#             self.sqrt_alphas_cumprod, target_t, x_start.shape
#         )
#         sqrt_one_minus_alphas_cumprod_t = extract(
#             self.sqrt_one_minus_alphas_cumprod, target_t, x_start.shape
#         )
...
#         betas_t = extract(
#             self.betas, t, x_image.shape
#         )  # t까지의 betas 얻어오기 shape = [64, 1, 1, 1]
#         sqrt_one_minus_alphas_cumprod_t = extract(
#             self.sqrt_one_minus_alphas_cumprod, t, x_image.shape
#         )  # shape = [64, 1, 1, 1]
#         sqrt_recip_alphas_t = extract(
#             self.sqrt_recip_alphas, t, x_image.shape
#         )  # shape = [64, 1, 1, 1]
...
#         batch_imgs = torch.randn(
#             shape, device=device
#         )  # shape=(batch_size, channels, img_size, img_size) [4,1,28,28]
#         total_imgs = []
#         for i in tqdm(
#             reversed(range(0, Diffusion.t_timesteps)),
#             desc="sampling loop time step",
#             total=Diffusion.t_timesteps,
#         ):
#             batch_imgs = self.p_sample(
#                 model,
#                 batch_imgs,
...
#             loss = diffusion_model.p_losses(
#                 model, image_batch, t, noise=None, loss_type="huber"
#             )
...
#                     map(
#                         lambda n: diffusion_model.sample(
#                             model,
#                             image_size,
#                             batch_size=n,
#                             channels=channels,
...
#         model, image_size=image_size, batch_size=64, channels=channels
#     )
